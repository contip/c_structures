...
void ll_init_tests()
{
    ...
    struct LL* new_1 = ll_init(array_test, array_test_size);
...
void ll_prepend_tests()
{
    /* test 1: initialize empty ll and build by adding values to front */
    struct LL* prepend_1 = ll_init(0, 0);
    ll_prepend(prepend_1, 1);
    ll_prepend(prepend_1, 2);
    ll_prepend(prepend_1, 3);
    ll_prepend(prepend_1, 4);
    ll_prepend(prepend_1, 5);
    ...
    struct LL* prepend_2 = ll_init(array_test, array_test_size);
    ll_prepend(prepend_2, 1);
    assert(prepend_2->head->val == 1);
    assert(prepend_2->count == 8);
    ll_prepend(prepend_2, 5);
...
void ll_append_tests()
{
    /* test 1: initialize empty ll and build by adding values to end */
    struct LL* append_1 = ll_init(0, 0);
    ll_append(append_1, 1);
    ll_append(append_1, 2);
    ll_append(append_1, 3);
    ll_append(append_1, 4);
    ll_append(append_1, 5);
    ...
    int array_test[7] = { 8, 6, 7, 5, 3, 0, 9 };
    int array_test_size = 7;
    struct LL* append_2 = ll_init(array_test, array_test_size);
    ll_append(append_2, -69);
    assert(append_2->tail->val == -69);
    assert(append_2->count == 8);
    ll_append(append_2, 1);
    ll_append(append_2, 2);
    ll_append(append_2, 3);
    ll_append(append_2, 4);
    ll_append(append_2, 5);
...
void ll_to_array_tests()
{
    ...
    int array_test[7] = { 8, 6, 7, 5, 3, 0, 9 };
    int array_test_size = 7;
    struct LL* to_array_1 = ll_init(array_test, array_test_size);
    ...
    struct LL* to_array_2 = ll_init(0, 0);
    ll_prepend(to_array_2, 69);
...
void ll_pop_end_tests()
{
    /* test 1: remove end until empty ll */
    int array_test[7] = { 8, 6, 7, 5, 3, 0, 9 };
    int array_test_size = 7;
    struct LL* pop_end_1 = ll_init(array_test, array_test_size);
    assert(ll_pop_end(pop_end_1) == 9);
    assert(pop_end_1->count == 6);
    assert(pop_end_1->tail->val == 0);
    assert(ll_pop_end(pop_end_1) == 0);
    assert(pop_end_1->count == 5);
    assert(pop_end_1->tail->val == 3);
    assert(ll_pop_end(pop_end_1) == 3);
    assert(pop_end_1->count == 4);
    assert(pop_end_1->tail->val == 5);
    assert(ll_pop_end(pop_end_1) == 5);
    assert(ll_pop_end(pop_end_1) == 7);
    assert(ll_pop_end(pop_end_1) == 6);
    assert(pop_end_1->count == 1);
    assert(pop_end_1->head->val == pop_end_1->tail->val &&
            pop_end_1->head->val == 8);
    assert(ll_pop_end(pop_end_1) == 8);
...
void ll_pop_front_tests()
{
    /* test 1: remove front until empty ll */
    int array_test[9] = { 5, -1, 2, 6, 9, 3, 7, 8, 0 };
    int i, array_test_size = 9;
    struct LL* pop_front_1 = ll_init(array_test, array_test_size);
    assert(ll_pop_front(pop_front_1) == 5);
    assert(pop_front_1->count == 8);
    assert(pop_front_1->head->val == -1);
    assert(ll_pop_front(pop_front_1) == -1);
    assert(pop_front_1->count == 7);
    assert(pop_front_1->head->val == 2);
    assert(ll_pop_front(pop_front_1) == 2);
    assert(pop_front_1->count == 6);
    assert(pop_front_1->head->val == 6);
    assert(ll_pop_front(pop_front_1) == 6);
    assert(ll_pop_front(pop_front_1) == 9);
    assert(ll_pop_front(pop_front_1) == 3);
    assert(ll_pop_front(pop_front_1) == 7);
    assert(ll_pop_front(pop_front_1) == 8);
    assert(pop_front_1->count == 1);
    assert(pop_front_1->head->val == pop_front_1->tail->val &&
            pop_front_1->head->val == 0);
    assert(ll_pop_front(pop_front_1) == 0);
...
void ll_remove_first_tests()
{
    /* test 1 */
    int array_test[9] = { 5, -1, 2, 6, 9, 3, -1, 8, 0 };
    int array_test_size = 9;
    struct LL* rem_first_1 = ll_init(array_test, array_test_size);
    assert(ll_remove_first(rem_first_1, 69) == false);
    assert(ll_remove_first(rem_first_1, -1) == true);
    assert(ll_contains(rem_first_1, -1) == true);
    assert(rem_first_1->head->val == 5);
    assert(rem_first_1->head->next->val == 2);
    assert(ll_remove_first(rem_first_1, -1) == true);
    assert(ll_remove_first(rem_first_1, -1) == false);
...
void ll_remove_index_tests()
{
    ...
    int array_test[7] = { 8, 6, 7, 5, 3, 0, 9 };
    int array_test_size = 7;
    struct LL* remove_index_1 = ll_init(array_test, array_test_size);
...
void ll_clear_tests()
{
    ...
    int array_test[7] = { 8, 6, 7, 5, 3, 0, 9 };
    int array_test_size = 7;
    struct LL* clear_1 = ll_init(array_test, array_test_size);
    ll_clear(clear_1);
...
{
    ll_init_tests();
    printf("All ll_init tests pass!\n");
    ll_prepend_tests();
    printf("All ll_prepend tests pass!\n");
    ll_append_tests();
    printf("All ll_append tests pass!\n");
    ll_to_array_tests();
    printf("All ll_to_array tests pass!\n");
    ll_pop_end_tests();
    printf("All ll_pop_end tests pass!\n");
    ll_pop_front_tests();
    printf("All ll_pop_front tests pass!\n");
    ll_contains_tests();
    printf("All ll_contains tests pass!\n");
    ll_remove_first_tests();
    printf("All ll_remove_first tests pass!\n");
    ll_remove_index_tests();
    printf("All ll_remove_index tests pass!\n");
    ll_clear_tests();
    printf("All ll_clear tests pass!\n");
    ll_splice_tests();
    printf("All ll_splice tests pass!\n");
    return 0;
}
